...
export function useAudioPlayer({
  ...
  useEffect(() => {
    ...
    const advanceAfter = (ms: number) => {
      ...
      // Allow external suppression via a global flag (used by the recording script).
      if ((window as Window & { __DISABLE_AUTO_ADVANCE__?: boolean }).__DISABLE_AUTO_ADVANCE__) return;
    ...
    // Listen for video-ended events dispatched by video-step components.
