...
export function useStepper(chapters: ChapterDef[]): StepperState {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      // In auto mode, Space is handled exclusively by useAutoMode (autoStartGate).
      // ArrowRight also doesn't advance in auto mode — audio drives the step.
      const inAutoMode = document.body.getAttribute('data-mode') === 'auto';
      if (e.key === " ") {
        // Space is reserved for AutoStartGate in auto mode — don't advance.
        e.preventDefault();
        if (!inAutoMode) next();
        ...
        return; // no manual prev/jump/chapters in auto mode
